...
* The celebrated [Fischer, Lynch, Paterson](https://groups.csail.mit.edu/tds/papers/Lynch/jacm85.pdf) lower bound says that in the asynchronous model, any protocol that solves consensus that withstands an adversary, that can fail-stop just one party, must have an infinite execution.
* Authenticated Byzantine Agreement is possible for $n>2f$ in the synchronous model but impossible for $n \leq 3f$ in the asynchronous model. 
<!-- ALIN: The sentence "any protocol that solves consensus that withstands an adversary, that can fail-stop just one party, must have an infinite execution." is long and difficult to read. I would rephrase as: "any protocol that solves consensus withstanding an adversary (who can fail-stopp just one party) must have infinite execution time." -->
<!-- ALIN: $f$ is not defined here... some readers might be confused. -->
...
<!-- ALIN: would simplify to "Many large scale systems use protocols that [...]" -->
