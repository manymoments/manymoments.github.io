...
$^{++}$ For the work on optimal good-case latency, the latency is described assuming an *unsynchronized start* between different parties. If different parties start at the same time, the latency is $\Delta + \delta$. In both cases, the latencies are optimal.
