...
At a very high level, the Dolev and Resichuk lower bound says that if you send few messages (less than $(f/2)^2$), then some honest party will receive no message! The party that receives no message has no way of reaching agreement with the rest. More technically, we use a trivial indistinguishability argument. We create a world where a node $p$ receives no message at all. Thus, node $p$ cannot distinguish between a world where the designated sender sends 0 vs a world where it sends 1.
