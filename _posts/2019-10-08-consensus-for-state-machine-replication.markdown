...
An example state machine is a ledger maintained by Bitcoin. The state consists of the set of public keys along with the associated Bitcoins. The input (or cmd) to the state machine is a transaction between parties. The log corresponds to the Bitcoin ledger. And the transition function apply is the function that determines whether a transaction is valid, and if yes, performs the desired transfer.
